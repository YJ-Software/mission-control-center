...
function LogList({
  ...
  const { data: logs = [], isLoading, refetch, isFetching } = useQuery<LogEntry[]>({
    queryKey: ['obsidian-logs', source],
    queryFn: () => fetch(`/api/second-brain/obsidian/logs?source=${source}&limit=300`).then(r => r.json()),
...
export function ObsidianLogs() {
  const t = useTranslations('secondBrain.obsidian.logs')
